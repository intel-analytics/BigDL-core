...
JNIEXPORT jintArray JNICALL PREFIX(setOmpAffinity0)(JNIEnv *env, jclass class, jintArray set)
{
...
#pragma omp parallel
  ...
  JNIEXPORT jobjectArray JNICALL PREFIX(getOmpAffinity0)(JNIEnv *env, jclass class,
                                                         jintArray set)
  {
    int *available_cores = (*env)->GetPrimitiveArrayCritical(env, set, JNI_FALSE);
    ...
    // create a 2-D array, which omp_num_threads row and available_cores_num column.
    jclass int_array_cls = (*env)->FindClass(env, "[I");
    jobjectArray result = (*env)->NewObjectArray(env, omp_num_threads, int_array_cls, NULL);
