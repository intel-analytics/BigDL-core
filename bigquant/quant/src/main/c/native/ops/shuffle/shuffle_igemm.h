...
#if defined(AVX512)
#include "../kernel/shuffle_avx512_igemm_4x4x64.h"
...
void ShuffleGEMM(int8_t* pa, uint8_t* pb, int* pc, size_t m, size_t n, size_t k,
  float fault_tolerance, size_t pad_m, size_t pad_n, GEMM_KERNEL kernel) {
                ...
                for (size_t y0 = 0; y0 < blocks[6]; y0 += blocks[8]) {
                  for (size_t x0 = 0; x0 < blocks[7]; x0 += blocks[9]) {
                    ...
                    if ((j_index < n) && (i_index < m)) {
                      ...
                      kernel(local_pa, local_pb, k, fault_tolerance, result, std::min(valid_m - i_index, kernel_m), std::min(valid_n - j_index, kernel_n));
                    }
                  }
                }
              }
...
template <size_t kernel_m, size_t kernel_n, size_t kernel_k, typename GEMM_KERNEL>
void InternalMixPrecisionGemm(ORDER order,
  ...
  ShuffleGEMM<kernel_m, kernel_n, kernel_k>(pad_a, pad_b, c, m_out, n_out, k_out, fault_tolerance, m_out - m, n_out - n, kernel);
#ifdef TIME_PROFILE
...
static INLINE_SPECIFIER void INLINE_ATTRIBUTE NNFixPointGemmSelect(int8_t* &pa, uint8_t* &pb, size_t k, float fault_tolerance, float* result[], size_t length, size_t valid_lanes,
                        ...
                        bool is_block) {
#if defined(AVX512)
  if ((kernel_m == 4) && (kernel_n == 4) && (kernel_k == 64)) {
    kernel::avx512_igemm4x4x64::ApplyKernelWrapper<kernel_m, kernel_n, kernel_k, layout>(pa, pb, k, fault_tolerance, result, length, valid_lanes, i_index, j_index,
  ...
  if ((kernel_m == 4) && (kernel_n == 8) && (kernel_k == 8)) {
    kernel::igemm4xn::ApplyKernelWrapper<kernel_m, kernel_n, kernel_k, layout>(pa, pb, k, fault_tolerance, result, length, valid_lanes, i_index, j_index,
                                                                        ratio_a, ratio_b, min_b, kernel_sum, bias,
                                                                        conv_relu_fusion, conv_bn_fusion, conv_bn_relu_fusion, conv_relu_bn_fusion,
                                                                        global_mean, mul_variance_coeff, scale, shift,
                                                                        is_block);
  ...
  if ((kernel_m == 4) && (kernel_n == 1) && (kernel_k == 32)) {
    kernel::igemm4x1::ApplyKernelWrapper<kernel_m, kernel_n, kernel_k, layout>(pa, pb, k, fault_tolerance, result, length, valid_lanes, i_index, j_index,
                                                                        ratio_a, ratio_b, min_b, kernel_sum, bias,
                                                                        conv_relu_fusion, conv_bn_fusion, conv_bn_relu_fusion, conv_relu_bn_fusion,
                                                                        global_mean, mul_variance_coeff, scale, shift,
                                                                        is_block);
  ...
  */
  if ((kernel_m == 2) && (kernel_n == 2) && (kernel_k == 16)) {
    kernel::sse42_igemm2x2x16::ApplyKernelWrapper<kernel_m, kernel_n, kernel_k, layout>(pa, pb, k, fault_tolerance, result, length, valid_lanes, i_index, j_index,
                                                                        ratio_a, ratio_b, min_b, kernel_sum, bias,
                                                                        conv_relu_fusion, conv_bn_fusion, conv_bn_relu_fusion, conv_relu_bn_fusion,
                                                                        global_mean, mul_variance_coeff, scale, shift,
                                                                        is_block);
...
static INLINE_SPECIFIER bool INLINE_ATTRIBUTE NHWCRTGenrateTargetAddr(DType *result[], DType *pc, size_t valid_m, size_t valid_n, size_t i_index, size_t j_index, \
...
#if defined(AVX512)
  if ((kernel_m == 4) && (kernel_n == 4) && (kernel_k == 64)) {
    return kernel::avx512_igemm4x4x64::NHWCRTGenrateTargetAddr<DType, kernel_m, kernel_n, kernel_k>(result, pc, valid_m, valid_n, i_index, j_index, cur_group, channel_per_group, total_channels);
  }
#elif defined(__AVX2__)
  if ((kernel_m == 4) && (kernel_n == 8) && (kernel_k == 8)) {
    return kernel::igemm4xn::NHWCRTGenrateTargetAddr<DType, kernel_m, kernel_n, kernel_k>(result, pc, valid_m, valid_n, i_index, j_index, cur_group, channel_per_group, total_channels);
  }
  if ((kernel_m == 4) && (kernel_n == 1) && (kernel_k == 32)) {
    //return kernel::igemm4x1::NHWCRTGenrateTargetAddr<DType, kernel_m, kernel_n, kernel_k>(result, pc, valid_m, valid_n, i_index, j_index, cur_group, channel_per_group, total_channels);
  }
#else
  /*
  if ((kernel_m == 4) && (kernel_n == 4) && (kernel_k == 8)) {
  ...
  */
  if ((kernel_m == 2) && (kernel_n == 2) && (kernel_k == 16)) {
    return kernel::sse42_igemm2x2x16::NHWCRTGenrateTargetAddr<DType, kernel_m, kernel_n, kernel_k>(result, pc, valid_m, valid_n, i_index, j_index, cur_group, channel_per_group, total_channels);
  }
#endif
}
...
static INLINE_SPECIFIER int INLINE_ATTRIBUTE NCHWRTGenrateTargetAddr(DType *result[], DType *pc, size_t valid_m, size_t valid_n, size_t i_index, size_t j_index, \
...
#if defined(AVX512)
  if ((kernel_m == 4) && (kernel_n == 4) && (kernel_k == 64)) {
    return kernel::avx512_igemm4x4x64::NCHWRTGenrateTargetAddr<DType, kernel_m, kernel_n, kernel_k>(result, pc, valid_m, valid_n,
  ...
  if ((kernel_m == 4) && (kernel_n == 1) && (kernel_k == 32)) {
    return kernel::igemm4x1::NCHWRTGenrateTargetAddr<DType, kernel_m, kernel_n, kernel_k>(result, pc, valid_m, valid_n,
                                                                                  i_index, j_index, cur_group,
                                                                                  feature_map_size_per_image, feature_map_size_per_group,
                                                                                  feature_map_size_per_channel);
  }
  ...
  */
  if ((kernel_m == 2) && (kernel_n == 2) && (kernel_k == 16)) {
    return kernel::sse42_igemm2x2x16::NCHWRTGenrateTargetAddr<DType, kernel_m, kernel_n, kernel_k>(result, pc, valid_m, valid_n,
                                                                                  i_index, j_index, cur_group,
                                                                                  feature_map_size_per_image, feature_map_size_per_group,
                                                                                  feature_map_size_per_channel);
  }
                ...
                for (size_t y0 = 0; y0 < blocks[6]; y0 += blocks[8]) {
                  for (size_t x0 = 0; x0 < blocks[7]; x0 += blocks[9]) {
                    ...
                    if ((j_index < n) && (i_index < m)) {
                      ...
                      bool is_block;
                      if (layout == NCHW) {
                        is_block = NCHWRTGenrateTargetAddr<float, kernel_m, kernel_n, kernel_k>(result, pc, valid_m, valid_n, i_index, j_index, cur_group, feature_map_size_per_image, feature_map_size_per_group, feature_map_size_per_channel);
                      } else {
                        is_block = NHWCRTGenrateTargetAddr<float, kernel_m, kernel_n, kernel_k>(result, pc, valid_m, valid_n, i_index, j_index, cur_group, channel_per_group, total_channels);
                      }
                      NNFixPointGemmSelect<kernel_m, kernel_n, kernel_k, layout>(local_pa, local_pb, k, fault_tolerance, result, std::min(valid_m - i_index, kernel_m), std::min(valid_n - j_index, kernel_n), i_index, j_index, ratio_a, ratio_b, min_b, kernel_sum, bias, conv_relu_fusion, conv_bn_fusion, conv_bn_relu_fusion, conv_relu_bn_fusion, global_mean, mul_variance_coeff, scale, shift, is_block);
                    }
                  }
                }
              }
