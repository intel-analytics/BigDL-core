...
void InternalQuantizedConvOpSetupConvParameter(QuantizedConvOp* p, LAYOUT layout, size_t channel_out, size_t channel_in, size_t group, size_t kernel_h, size_t kernel_w, \
  ...
  reinterpret_cast<ConvOp*>(p)->SetupConvolutionParameter(layout, channel_out, channel_in, group, kernel_h, kernel_w, stride_h, stride_w,  pad_h, pad_w, dialation_h, dialation_w, fusion_mask, algo);
}
...
void InternalQuantizedConvOpExecute(QuantizedConvOp* p, float *dst, float *data, float *bias, size_t batch_size, size_t channel_in, size_t height_in, size_t width_in) {
...
void InternalQuantizedFCOpSetupFCParameter(QuantizedFCOp* p, LAYOUT layout, size_t channel_out, size_t channel_in, FC_ALGORITHM algo) {
...
void InternalQuantizedFCOpExecute(QuantizedFCOp* p, float* dst, float *data, float *bias, size_t batch_size, size_t channel_in) {
...
void InternalQuantizedConvKernelDescInit(QuantizedTensorDesc *quantized_tensor, size_t c_out, size_t c_in, size_t kernel_h, size_t kernel_w) {
...
void InternalQuantizedConvKernelInit(QuantizedTensorDesc *quantized_tensor, float *src, size_t c_out, size_t c_in, size_t kernel_h, size_t kernel_w, float threshold, LAYOUT layout) {
  float *tmp;
  if (layout == NHWC) {
    tmp = src;
  } else {
  ...
  shuffle::PadQuantizeShuffle2D<float, CONV_SHUFFLE_KERNEL_M, CONV_SHUFFLE_KERNEL_K>(reinterpret_cast<int8_t*>(quantized_tensor->data), quantized_tensor->ori_shape[0], quantized_tensor->ori_shape[1],
    GetAlignmentLength(quantized_tensor->ori_shape[0], CONV_SHUFFLE_KERNEL_M), GetAlignmentLength(quantized_tensor->ori_shape[1], CONV_SHUFFLE_KERNEL_K),
    tmp, reinterpret_cast<float*>(quantized_tensor->min), reinterpret_cast<float*>(quantized_tensor->max), reinterpret_cast<float*>(quantized_tensor->ratio), threshold);
  if (layout == NCHW) {
    aligned_free(tmp);
  }
}

void InternalQuantizedConvKernelLoadFromModel(QuantizedTensorDesc *quantized_tensor, int8_t *src, float *min, float* max, size_t c_out, size_t c_in, size_t kernel_h, size_t kernel_w, float threshold, LAYOUT layout) {
  aligned_malloc(&(quantized_tensor->min), 64, quantized_tensor->workspace_size_per_meta_info);
  aligned_malloc(&(quantized_tensor->max), 64, quantized_tensor->workspace_size_per_meta_info);
  aligned_malloc(&(quantized_tensor->ratio), 64, quantized_tensor->workspace_size_per_meta_info);
  aligned_malloc(&(quantized_tensor->data), 64, quantized_tensor->workspace_size);
...
void InternalQuantizedConvDataDescInit(QuantizedTensorDesc *quantized_tensor, size_t c_in, size_t kernel_h, size_t kernel_w, size_t stride_h, size_t stride_w, size_t pad_h, size_t pad_w, size_t dilation_h, size_t dilation_w, size_t batch_size, size_t h_in, size_t w_in) {
...
void InternalQuantizedConvDataInit(QuantizedTensorDesc *quantized_tensor, float *src, size_t c_in, size_t kernel_h, size_t kernel_w, size_t stride_h, size_t stride_w, size_t pad_h, size_t pad_w, size_t dilation_h, size_t dilation_w, size_t batch_size, size_t h_in, size_t w_in, float threshold, LAYOUT layout) {
  size_t h_out = GetConvOutSize(h_in, kernel_h, stride_h, pad_h, dilation_h);
  size_t w_out = GetConvOutSize(w_in, kernel_w, stride_w, pad_w, dilation_w);
...
void InternalQuantizedFCKernelInit(QuantizedTensorDesc *quantized_tensor, float *src, size_t c_out, size_t c_in, float threshold, LAYOUT layout) {
  assert((layout == NCHW) || (layout == NHWC));
  shuffle::PadQuantizeShuffle2D<float, FC_SHUFFLE_KERNEL_M, FC_SHUFFLE_KERNEL_K>(reinterpret_cast<int8_t*>(quantized_tensor->data), quantized_tensor->ori_shape[0], quantized_tensor->ori_shape[1],
    GetAlignmentLength(quantized_tensor->ori_shape[0], FC_SHUFFLE_KERNEL_M), GetAlignmentLength(quantized_tensor->ori_shape[1], FC_SHUFFLE_KERNEL_K),
    src, reinterpret_cast<float*>(quantized_tensor->min), reinterpret_cast<float*>(quantized_tensor->max), reinterpret_cast<float*>(quantized_tensor->ratio), threshold);
}

void InternalQuantizedFCKernelLoadFromModel(QuantizedTensorDesc *quantized_tensor, int8_t *src, float *min, float* max, size_t c_out, size_t c_in, float threshold, LAYOUT layout) {
  assert((layout == NCHW) || (layout == NHWC));
  aligned_malloc(&(quantized_tensor->min), 64, quantized_tensor->workspace_size_per_meta_info);
  aligned_malloc(&(quantized_tensor->max), 64, quantized_tensor->workspace_size_per_meta_info);
  aligned_malloc(&(quantized_tensor->ratio), 64, quantized_tensor->workspace_size_per_meta_info);
  ...
  shuffle::PadQuantizeShuffle2D<float, FC_SHUFFLE_KERNEL_M, FC_SHUFFLE_KERNEL_K>(reinterpret_cast<int8_t*>(quantized_tensor->data), quantized_tensor->ori_shape[0], quantized_tensor->ori_shape[1],
    GetAlignmentLength(quantized_tensor->ori_shape[0], FC_SHUFFLE_KERNEL_M), GetAlignmentLength(quantized_tensor->ori_shape[1], FC_SHUFFLE_KERNEL_K),
    fp_model.data(), reinterpret_cast<float*>(quantized_tensor->min), reinterpret_cast<float*>(quantized_tensor->max), reinterpret_cast<float*>(quantized_tensor->ratio), threshold);
}
...
void InternalQuantizedFCDataInit(QuantizedTensorDesc *quantized_tensor, float *src, size_t batch_size, size_t channel, float threshold, LAYOUT layout) {
  assert((layout == NCHW) || (layout == NHWC));
  shuffle::PadQuantizeShuffle2D<float, FC_SHUFFLE_KERNEL_N, FC_SHUFFLE_KERNEL_K>(reinterpret_cast<uint8_t*>(quantized_tensor->data), batch_size, channel,
