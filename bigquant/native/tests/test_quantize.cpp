...
TEST(Quantize, PADQuantizeShuffle2DUInt8) {
  ...
  for (auto problem_it = problemset.begin(); problem_it < problemset.end(); ++problem_it) {
    for (auto thres_it = thresholds.begin(); thres_it < thresholds.end(); ++thres_it) {
      ...
      shuffle::PadQuantizeShuffle2D<float, m_block, n_block>(dst.data(), m, n, pad_m, pad_n, src.data(), min.data(),
                                                             max.data(), ratio.data(), *thres_it);
      ...
      for (size_t i = 0; i < m; ++i) {
        ...
        for (size_t j = 0; j < n; ++j) {
          ...
          BYTES_EQUAL(*(dst.data() + block_id * block_size + (i % m_block) * n_block + (j % n_block)),
                      static_cast<uint8_t>(std::round((*(src.data() + i * n + j) - min[i]) * ratio_ref[i])));
        }
      }
    }
  }
    ...
    for (auto thres_it = thresholds.begin(); thres_it < thresholds.end(); ++thres_it) {
      ...
      for (size_t i = 0; i < m; ++i) {
        ...
        for (size_t j = 0; j < n; ++j) {
          ...
          size_t block_id = y_block_id * x_block_num + x_block_id;
          BYTES_EQUAL(*(dst.data() + block_id * block_size + (i % m_block) * n_block + (j % n_block)),
                      static_cast<uint8_t>(std::round(*(src.data() + i * n + j) * ratio_ref[i])));
        }
      }
    }
  }
  ...
  for (auto thres_it = thresholds.begin(); thres_it < thresholds.end(); ++thres_it) {
    for (auto it = data.begin(); it < data.end(); ++it) {
      ...
      shuffle::PadQuantizeShuffleIm2colWrapper<float, NHWC>(src.data(), batch_size, channel, group, height_in, width_in,
                                                            kernel, kernel, pad, pad, stride, stride, 1, 1, dst_array,
                                                            min_array, max_arrary, ratio_array, NULL, 127.0f);

      shuffle::PadQuantizeShuffleIm2colRef<float, m_block, n_block, NHWC>(
          src.data(), batch_size, channel, height_in, width_in, kernel, kernel, pad, pad, stride, stride, 1, 1,
          dst_ref.data(), min_ref.data(), max_ref.data(), ratio_ref.data(), 127.0f);
  ...
  for (auto thres_it = thresholds.begin(); thres_it < thresholds.end(); ++thres_it) {
    for (auto it = data.begin(); it < data.end(); ++it) {
      ...
      shuffle::PadQuantizeShuffleIm2colWrapper<float, NCHW>(src.data(), batch_size, channel, group, height_in, width_in,
                                                            kernel, kernel, pad, pad, stride, stride, 1, 1, dst_array,
                                                            min_array, max_arrary, ratio_array, NULL, *thres_it);

      shuffle::PadQuantizeShuffleIm2colRef<float, m_block, n_block, NCHW>(
          src.data(), batch_size, channel, height_in, width_in, kernel, kernel, pad, pad, stride, stride, 1, 1,
          dst_ref.data(), min_ref.data(), max_ref.data(), ratio_ref.data(), *thres_it);
  ...
  for (auto thres_it = thresholds.begin(); thres_it < thresholds.end(); ++thres_it) {
    for (auto it = data.begin(); it < data.end(); ++it) {
      ...
      PadQuantizeIm2colWrapper<float, NHWC>(src.data(), batch_size, channel, group, height_in, width_in, \
          kernel, kernel, pad, pad, stride, stride, 1, 1, dst_array, min_array, max_arrary, ratio_array, NULL,
*thres_it);

      QuantizeIm2colRef<float, NHWC>(src.data(), batch_size, channel, height_in, width_in, \
          kernel, kernel, pad, pad, stride, stride, 1, 1, dst_ref.data(), min_ref.data(), max_ref.data(),
ratio_ref.data(), *thres_it);
