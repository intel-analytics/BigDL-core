...
static INLINE_SPECIFIER void INLINE_ATTRIBUTE KernelReduce(int8_t *&pa, uint8_t *&pb, SIMDSITYPE &c11, SIMDSITYPE &c12,
                                                           SIMDSITYPE &c21, SIMDSITYPE &c22, SIMDSITYPE &c31,
                                                           SIMDSITYPE &c32, SIMDSITYPE &c41, SIMDSITYPE &c42,
                                                           SIMDSITYPE &sum1, SIMDSITYPE &sum2, SIMDSITYPE &sum3,
                                                           SIMDSITYPE &sum4, SIMDSITYPE &threshold, SIMDSITYPE &ones,
...
#ifdef __AVX2__
static INLINE_SPECIFIER void INLINE_ATTRIBUTE AVX2Kernel4x8x8(int8_t *&pa, uint8_t *&pb, SIMDSITYPE &c11,
                                                              SIMDSITYPE &c12, SIMDSITYPE &c21, SIMDSITYPE &c22,
                                                              SIMDSITYPE &c31, SIMDSITYPE &c32, SIMDSITYPE &c41,
                                                              SIMDSITYPE &c42) {
  /*
  ...
  */
  ...
  SIMDSITYPE a1 = reinterpret_cast<SIMDSITYPE>(BROADCASTLOAD_PD(reinterpret_cast<double *>(pa)));
  c11 = ADDS_EPI16(c11, MADD_EPI8(b1, a1));
  c12 = ADDS_EPI16(c12, MADD_EPI8(b2, a1));

  SIMDSITYPE a2 = reinterpret_cast<SIMDSITYPE>(BROADCASTLOAD_PD(reinterpret_cast<double *>(pa + 8)));
  c21 = ADDS_EPI16(c21, MADD_EPI8(b1, a2));
  c22 = ADDS_EPI16(c22, MADD_EPI8(b2, a2));

  SIMDSITYPE a3 = reinterpret_cast<SIMDSITYPE>(BROADCASTLOAD_PD(reinterpret_cast<double *>(pa + 16)));
  c31 = ADDS_EPI16(c31, MADD_EPI8(b1, a3));
  c32 = ADDS_EPI16(c32, MADD_EPI8(b2, a3));

  SIMDSITYPE a4 = reinterpret_cast<SIMDSITYPE>(BROADCASTLOAD_PD(reinterpret_cast<double *>(pa + 24)));
  c41 = ADDS_EPI16(c41, MADD_EPI8(b1, a4));
  c42 = ADDS_EPI16(c42, MADD_EPI8(b2, a4));
...
#else
static INLINE_SPECIFIER void INLINE_ATTRIBUTE SSE42Kernel4x4x8(int8_t *&pa, uint8_t *&pb, SIMDSITYPE &c11,
                                                               SIMDSITYPE &c12, SIMDSITYPE &c21, SIMDSITYPE &c22,
                                                               SIMDSITYPE &c31, SIMDSITYPE &c32, SIMDSITYPE &c41,
                                                               SIMDSITYPE &c42) {
...
static INLINE_SPECIFIER void INLINE_ATTRIBUTE PostHaddReduce(SIMDSITYPE &c11, SIMDSITYPE &c12, SIMDSITYPE &c21,
                                                             SIMDSITYPE &c22, SIMDSITYPE &c31, SIMDSITYPE &c32,
                                                             SIMDSITYPE &c41, SIMDSITYPE &c42, SIMDSITYPE &sum1,
...
static INLINE_SPECIFIER void INLINE_ATTRIBUTE CommitBlockResult(SIMDSITYPE &sum1, SIMDSITYPE &sum2, SIMDSITYPE &sum3,
                                                                SIMDSITYPE &sum4, void *result[], size_t length,
                                                                size_t valid_lanes) {
...
static INLINE_SPECIFIER void INLINE_ATTRIBUTE CommitResult(SIMDSITYPE &sum1, SIMDSITYPE &sum2, SIMDSITYPE &sum3,
                                                           SIMDSITYPE &sum4, void *result[], size_t length,
                                                           size_t valid_lanes) {
#ifdef __AVX2__
  SIMDSITYPEHALF tmp1, tmp2, tmp3, tmp4;
  tmp1 = EXTRACT_SI128(sum1, 0);
  tmp2 = EXTRACT_SI128(sum2, 0);
  tmp3 = EXTRACT_SI128(sum3, 0);
  tmp4 = EXTRACT_SI128(sum4, 0);
  for (int ky = 0; ky < valid_lanes; ++ky) {
    ...
    if (ky == 3) {
      ...
      tmp3 = EXTRACT_SI128(sum3, 1);
      tmp4 = EXTRACT_SI128(sum4, 1);
    } else {
      tmp1 = SRLI_SI128_HALF(tmp1, 4);
      tmp2 = SRLI_SI128_HALF(tmp2, 4);
      tmp3 = SRLI_SI128_HALF(tmp3, 4);
      tmp4 = SRLI_SI128_HALF(tmp4, 4);
    }
  }
#else
  for (int ky = 0; ky < valid_lanes; ++ky) {
    ...
    sum3 = SRLI_SI128(sum3, 4);
    sum4 = SRLI_SI128(sum4, 4);
  }
#endif
}
...
static INLINE_SPECIFIER void INLINE_ATTRIBUTE NHWCFMABlockResult(
  ...
  SIMDPSTYPE mix1 = reinterpret_cast<SIMDPSTYPE>(
      UNPACKLO_PD(reinterpret_cast<SIMDPDTYPE>(result12lo), reinterpret_cast<SIMDPDTYPE>(result34lo)));
  SIMDPSTYPE mix2 = reinterpret_cast<SIMDPSTYPE>(
      UNPACKHI_PD(reinterpret_cast<SIMDPDTYPE>(result12lo), reinterpret_cast<SIMDPDTYPE>(result34lo)));
  SIMDPSTYPE mix3 = reinterpret_cast<SIMDPSTYPE>(
      UNPACKLO_PD(reinterpret_cast<SIMDPDTYPE>(result12hi), reinterpret_cast<SIMDPDTYPE>(result34hi)));
  SIMDPSTYPE mix4 = reinterpret_cast<SIMDPSTYPE>(
      UNPACKHI_PD(reinterpret_cast<SIMDPDTYPE>(result12hi), reinterpret_cast<SIMDPDTYPE>(result34hi)));
#endif
#ifdef __AVX2__
...
FMAResult(SIMDSITYPE &sum1, SIMDSITYPE &sum2, SIMDSITYPE &sum3, SIMDSITYPE &sum4, float *result[], size_t length,
  ...
  for (size_t ky = 0; ky < valid_lanes; ++ky) {
    if (length == 4) {
      ...
      *(result[1 * kernel_n + ky]) = ratio_a[i_index + 1] * ratio_b[j_index + ky] * EXTRACT_EPI32_HALF(tmp2, 0) +
                                     kernel_sum[i_index + 1] * min_b[j_index + ky] + bias2;
      *(result[2 * kernel_n + ky]) = ratio_a[i_index + 2] * ratio_b[j_index + ky] * EXTRACT_EPI32_HALF(tmp3, 0) +
                                     kernel_sum[i_index + 2] * min_b[j_index + ky] + bias3;
      *(result[3 * kernel_n + ky]) = ratio_a[i_index + 3] * ratio_b[j_index + ky] * EXTRACT_EPI32_HALF(tmp4, 0) +
...
static INLINE_SPECIFIER void INLINE_ATTRIBUTE ApplyKernel(int8_t *&pa, uint8_t *&pb, size_t k, float fault_tolerance,
  ...
  while (k >= UNROLL_NUM * kernel_k) {
    KernelReduce(pa, pb, c11, c12, c21, c22, c31, c32, c41, c42, sum1, sum2, sum3, sum4, max_threshold, ones, kernel,
                 sum);
...
static INLINE_SPECIFIER void INLINE_ATTRIBUTE ApplyKernelWrapper(int8_t *&pa, uint8_t *&pb, size_t k,
  ...
  if ((length >= kernel_m) && (valid_lanes >= kernel_n)) {
    ApplyKernel<kernel_k>(pa, pb, k, fault_tolerance, result, kernel_m, kernel_n, AVX2Kernel4x8x8, HaddPairReduce,
                          PostHaddReduce, CommitBlockResult);
  } else {
    ApplyKernel<kernel_k>(pa, pb, k, fault_tolerance, result, length, valid_lanes, AVX2Kernel4x8x8, HaddPairReduce,
                          PostHaddReduce, CommitResult);
  }
#else
  assert((kernel_m == 4) && (kernel_n == 4) && (kernel_k == 8));
  if ((length >= kernel_m) && (valid_lanes >= kernel_n)) {
    ApplyKernel<kernel_k>(pa, pb, k, fault_tolerance, result, kernel_m, kernel_n, SSE42Kernel4x4x8, HaddPairReduce,
                          PostHaddReduce, CommitBlockResult);
  } else {
    ApplyKernel<kernel_k>(pa, pb, k, fault_tolerance, result, length, valid_lanes, SSE42Kernel4x4x8, HaddPairReduce,
                          PostHaddReduce, CommitResult);
  }
#endif
}
