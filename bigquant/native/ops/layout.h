...
void TransposeWrapper(const char ordering, const char trans, size_t rows, size_t cols, const DType alpha, DType * AB,
size_t lda, size_t ldb) {
  std::cerr << "Unimplemented" << std::endl;
}

template<>
void TransposeWrapper<float>(const char ordering, const char trans, size_t rows, size_t cols, const float alpha, float *
AB, size_t lda, size_t ldb) {
  mkl_simatcopy(ordering, trans, rows, cols, alpha, AB, lda, ldb);
}

template<>
void TransposeWrapper<double>(const char ordering, const char trans, size_t rows, size_t cols, const double alpha,
double * AB, size_t lda, size_t ldb) {
  mkl_dimatcopy(ordering, trans, rows, cols, alpha, AB, lda, ldb);
}

template <typename DType>
void TransformLayoutInPlace(LAYOUT dst_layout, LAYOUT src_layout, DType *src, size_t batch_size, size_t channels, size_t
hxw) {
...
void Transpose(DType *dst, DType *src, size_t m, size_t n) {
  ...
  std::cerr << "layout transform " << diff.count() << "us"
            << " " << m << " " << n << std::endl;
#endif
}
...
#pragma omp parallel for collapse(2) schedule(static, 1)
  ...
  auto diff = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
  std::cerr << "layout transform " << diff.count() << "us"
            << " " << batch_size << " " << channels << " " << hxw << std::endl;
#endif
}
